    ...
    with open(filepath, 'r') as f:
        ...
        for line in f:
            ...
            if in_data_section and not line.startswith('@'):
                ...
                label = parts[-1].strip()
                # Map positive/1 -> 1, negative/0 -> 0
                label_lower = label.lower()
                label_val = 1 if label_lower in ('positive', '1', '1.0', 'true', 'yes') else 0
...
def evaluate_models(base_path, method, dataset, models_to_run, random_seed=8):
    """
    ...
    for i in range(1, 6):
        # base_path already fully resolves to the dataset directory containing the split files.
        train_file = os.path.join(base_path, dataset+f"-{i}tra.dat")

        test_file = os.path.join(base_path, dataset+f"-{i}tst.dat")
...
def run_experiment(run_knn_model=False, run_rf_model=False, run_svm_model=False, file_path = r'D:\404-Brain-not-found', csv_name="result",csv_path = r'D:\404-Brain-not-found', method="-", dataset="-"):
    models_to_run = {}
    ...
    results_df = evaluate_models(file_path, method, dataset, models_to_run, random_seed=8)
    ...
    results_df.insert(1, 'Dataset', csv_name.split("_result_")[1])  # Extract dataset name from csv_name
